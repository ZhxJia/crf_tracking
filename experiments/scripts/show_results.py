...
from experiments.show import deep_sort_app
from experiments.deep_sort.iou_matching import iou
from experiments.application_util import visualization
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="data/MOT17Det/train/MOT17-05", required=True)
    parser.add_argument(
        "--result_file", help="Tracking output in MOTChallenge file format.",
        default="data/MOT17Det/train/MOT17-05/gt/gt.txt", required=True)
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    for sequence_txt in os.listdir(mot_dir):
        sequence = os.path.splitext(sequence_txt)[0]
        sequence_dir = os.path.join(mot_dir, sequence)
        if not os.path.exists(sequence_dir):
            continue
        # result_file = os.path.join(result_dir, sequence_txt) + "-FRCNN/gt/gt.txt"
        result_file = os.path.join(result_dir, sequence_txt) + "-FRCNN.txt"

        output_file = os.path.join(output_file_dir, f"{sequence}-FRCNN.avi")
        # run(
        #     args.sequence_dir, args.result_file, args.show_false_alarms,
        #     args.detection_file, args.update_ms, args.output_file)
        run(
            sequence_dir, result_file, False,
