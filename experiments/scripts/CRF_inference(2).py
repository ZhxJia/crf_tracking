...
def get_disp(delat_x, delta_y):
    disp = math.sqrt(delat_x ** 2 + delta_y ** 2)
    return disp
...
for track_id, id_info in track_dict.items():
    ...
    for id_frame, id_frame_info in id_info.items():
        ...
        # calculate the displacement from frame t-1 to frame t
        disp_t = get_disp(v_t[0], v_t[1])
        disp_t_1 = get_disp(v_t_1[0], v_t_1[1])
        if disp_t_1 == 0:
            raise ZeroDivisionError
        ddisp = disp_t / disp_t_1
        ...
        all_info[id_frame].append([track_id, dv[0], dv[1], dl[0], dl[1], dr, ddisp, score, int(id_frame_info[-1])])
        id_frame_info_t_2 = id_frame_info_t_1
        id_frame_info_t_1 = id_frame_info
...
def calc_binary(F_info, Wb):  # F_info is a list of two lists
    ...
    if (scorei > theta and scorej > theta):
        ...
        Pb_not_11 = 0
    return np.array([[exp(-Wb * Pb_11), exp(-Wb * Pb_not_11)], [exp(-Wb * Pb_not_11), exp(-Wb * Pb_11)]])
...
for test_frame_id in range(frame[0], frame[1]):
    ...
    for i in range(len(node_id_list)):
        ...
        for j in range(len(node_id_list)):
            ...
            g.factor([node_i, node_j], potential=calc_binary([frame_info[i], frame_info[j]], Wb=0.02))
